...
void creationTableau(int score, int meilleurScore, int distance, SDL_Surface* &fondTableauDeScore){
    ...
    char charScore[30], charMeilleurScore[30], charDistance[30];
    ...
    strcpy(charMeilleurScore, strMeilleurScore.c_str());

    std::string strDistance = "Distance : "+std::to_string(distance)+" (?)";
    strcpy(charDistance, strDistance.c_str());

    //creation des message 
    indicationDistance = TTF_RenderText_Blended( font, charDistance, textColor ); 
    indicationScore = TTF_RenderText_Blended( font, charScore, textColor ); 
    indicationMeilleurScore = TTF_RenderText_Blended( font, charMeilleurScore, textColor ); 
...
int main(int argc, char** argv) {
    ...
    font = TTF_OpenFont( "/home/yaelle/Documents/S2/Synthèse image/GLImac-Template/assets/font/retro.ttf", 15 ); 
